...
# -> 아침마다 15줄 코드를 작성 작성 및 실행(비효율적)
# => 15줄 코드를 함수로 정의하고 함수를 실행(효울적)

#함수 개발 가이드라인
#1. 함수이름
# + 함수내용은 가능하면 짧게 작성(line 줄이기)
# 함수이름에 함수의 역할과 의도를 명확히 드러낼 것
# ex) def print_hello:
        #print("Hello")
...
# 복잡한 로직이 사용되었을 때 식별 가능한 이름의 함수로 생성

# 함수의 종류
# 1. 내장함수(Bulit-in function)
#  python에서 내부적으로 제공하는 함수
#  ex) print(), len(), type() 등등
...
#함수 정의
# 1. 기본 함수 문법
# def 함수명(parameer1, parameter2, ...):
#     실행문
#     return 반환값

# 2. 함수 정의시 "def" 키워드 사용(define)
# 3. 인자(argumet or parameter) 정의 : 함수 입력 값
# 4. return: 함수 종료 의미
...
#2. 함수 호출
# 함수 호출문을 실행하며 함수가 종료되고 호출했던 곳으로 돌아옴
...
# Varaiable length parameter(가변 길이 인자)
# 인자(parameter) : 함수 입력값
# 함수 입력값의 갯수가 함수를 호출할 때마다 매변 다른 경우
# 가변길이 인자를 사용해야함
# ex) format(), print()
# print("hi","hello")
